...
function SignIn({ onSignIn, onSwitchToSignUp }) {
  ...
  return (
    <AuthShell title="Welcome back" subtitle="Login to your TeamSync workspace.">
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <button type="submit" className="btn btn-primary btn-lg" style={{ marginTop: 8 }} disabled={loading}>
          {loading ? 'Logging in…' : <>Login <IconArrowRight size={14} /></>}
        </button>
...
function SignUp({ onSignUp, onSwitchToSignIn }) {
  ...
  const validate = () => {
    ...
    else if (!/[^a-zA-Z0-9]/.test(form.password)) e.password = 'At least one special character (e.g., @, #, !)';
